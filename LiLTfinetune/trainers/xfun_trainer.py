    ...
    def create_optimizer(self, speedup_r=4.):
        if self.optimizer is None:
            ...
            for name in decay_parameters:
                if 'gru' in name or 'key_type_ffn' in name:
                    decoder_name = 'gare'
            ...
            if decoder_name == 'gare':
                ...
                """
                decay_parameters = [name for name in decay_parameters if "bias" not in name and 'key_attn' not in name and 'value_attn' not in name and 'gru' not in name ]
                # speedup_parameters = [name for name in get_parameter_names(self.model, []) if 'extractor' in name and 'rel_classifier' not in name]
                speedup_parameters = [name for name in get_parameter_names(self.model, []) if 'extractor' in name and ('key_attn' not in name and 'value_attn' not in name and 'gru' not in name )]
                speedup_parameters_1 = [name for name in get_parameter_names(self.model, []) if ('key_attn' in name or 'value_attn' in name or 'gru' in name) and 'extractor' in name]
                """

                decay_parameters = [name for name in decay_parameters if "bias" not in name and 'attn' not in name and 'gru' not in name and 'hgat' not in name  ]
                # speedup_parameters = [name for name in get_parameter_names(self.model, []) if 'extractor' in name and 'rel_classifier' not in name]
                speedup_parameters = [name for name in get_parameter_names(self.model, []) if 'extractor' in name and ('attn' not in name  and 'gru' not in name and 'hgat' not in name )]
                ...
                speedup_parameters_2 = [name for name in get_parameter_names(self.model, []) if ('hgat' in name ) and 'extractor' in name]
                ...
                optimizer_grouped_parameters = [
                    ...
                    {
                        "params": [p for n, p in self.model.named_parameters() if n not in decay_parameters and n not in speedup_parameters and n not in speedup_parameters_1 and n not in n not in speedup_parameters_2],
                        "weight_decay": 0.0,
                        "lr": self.args.learning_rate,
                    },
                    {
                    ...
                    {
                        "params": [p for n, p in self.model.named_parameters() if n in speedup_parameters_2],
                        "weight_decay": self.args.weight_decay,
                        "lr": self.args.learning_rate*self.attn_lr,
                    },
