...
"""
from transformers.models.bert.modeling_bert import BertEncoder,BertAttention 
class AdapterLayer(nn.Module):
    pass 

class Adapter(nn.Module):
    def __init__(self,args,config)：
        super(Adapter,self).__init()
        self.config = config
        self.args = args
        ...
        class AdapterConfig:
            ...
            attentio_probs_dropout_prob:float=0.1
        ...
        self.adapter = nn.ModuleList([AdapterLayer(self.args,AdapterConfig) for _ in range(self.adapter_num)])
        ...
        self.dropout = nn.Dropout(self.config.hidden_droput_prob)
        self.outproj = nn.Linear(self.config.hidden_size,args.max_seq_length)
    
    def forward(self,pretrained_model_outputs,input_ids,attention_mask=None,token_type_ids=None,position_ids=None,head_mask=None,labels=None):
...
class LayoutLMv2ForRelationExtraction(LayoutLMv2PreTrainedModel):
    def __init__(self, config, gare_args):
        super().__init__(config)
        self.save_result = gare_args.save_result
        self.save_dir = gare_args.save_dir
        self.checkpoint_type = gare_args.checkpoint_type
        #self.overwrite_result_file = gare_args.overwrite_result_file
        self.overwrite_result_file=True
        self.input_type = gare_args.input_type
        ...
        self.decoder = gare_args.decoder_name 
        if self.decoder == 're':
            self.extractor = REDecoder(config)
        elif self.decoder == 'gare':   
            self.extractor = GARE(gare_args)
    ...
    def forward(
        ...
        if self.use_adapter:
            ...
            with torch.no_grad():
                layoutflow_input_ids,textflow_input_ids = input_ids,input_ids.clone().detach()
                layoutflow_bbox,textflow_bbox = bbox,bbox.clone().detach()
                layout_input_ids_mask = layoutflow_input_ids != 1
                layoutflow_input_ids[layout_input_ids_mask] = 3 # set layoutflow_input_ids to <unk> 
                textflow_bbox[:,:,:] = 0 #set textflow_bbox to [0,0,0,0]

                layoutflow_outputs = self.layoutlmv2(
                    input_ids=layoutflow_input_ids,
                    bbox=layoutflow_bbox,
                    image=image,
                    attention_mask=attention_mask,
                    token_type_ids=token_type_ids,
                    position_ids=position_ids,
                ...
                textflow_outputs = self.layoutlmv2(
                    input_ids=textflow_input_ids,
                    bbox=textflow_bbox,
                    image=image,
                    attention_mask=attention_mask,
                    token_type_ids=token_type_ids,
            ...
            textflow_outputs = text_adapter(textflow_outputs)
            layoutflow_outputs = layout_adapter(layoutflow_outputs)
        ...
        if self.save_result:
            ...
            try:
                #from IPython import embed;embed()
                result_list = [pred_relations,relations]
                result_name_list = [self.checkpoint_type , 'label']
                path_list = [f'{name}.json' for name in result_name_list]

                for index,task in enumerate(result_list):
                    data = {
                        "input_ids":input_ids,
                        "bbox":bbox,
                        "entities":entities}
                    result_file_path = os.path.join(f'./pred_result/{self.save_dir}',path_list[index])
                    if self.overwrite_result_file and os.path.exists(result_file_path):
                        os.remove(result_file_path)
                        self.overwrite_result_file=False
                        print(f'=============={result_file_path} removed ==================' )
                    data['relations']=result_list[index]
                    data['input_ids'] = data['input_ids'].tolist()
                    data['bbox'] = data['bbox'].tolist()
                    if not os.path.exists(f'./pred_result/{self.save_dir}'):
                        os.mkdir(f'./pred_result/{self.save_dir}')
                ...
                print('result saving encount error')
